...
var expGuiCourse = function (pObject, config) {
    ...
    */
     ...
     * 結果一覧の表示フラグを取得
     */
    this.getViewCourseListFlag = function () {
        return viewCourseListFlag;
    };

    /**
     * 選択中の経路NOを取得
    */
    this.getSelectNo = function () {
        return selectNo;
    };
    ...
    */
    function viewResultRoute(courseObj, routeNo) {
        ...
        if (typeof callBackFunctionBind['select'] == 'function') {
            ...
            buffer += '<a class="exp_resultSelectButton" id="' + baseId + ':courseSelect:' + String(routeNo) + '" href="Javascript:void(0);"><span class="exp_text" id="' + baseId + ':courseSelect:' + String(routeNo) + ':text">マップ表示</span></a>';
            buffer += '</div>';
            buffer += '</div>';
        }
        buffer += '</div>';
